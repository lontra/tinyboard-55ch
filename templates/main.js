...
{% if config.stylesheets_board %}
	...
	var matches = document.URL.match(/\/(\w+)\/($|{{ config.dir.res|replace({'/': '\\/'}) }}{{ config.file_page|replace({'%d': '\\d+', '.': '\\.'}) }}|{{ config.file_index|replace({'.': '\\.'}) }}|{% if config.catalog_link %}{{ config.catalog_link|replace({'.': '\\.'}) }}{% else %}catalog\.html{% endif %}|{{ config.file_page|replace({'%d': '\\d+', '.': '\\.'}) }})/);
	{% raw %}
	if (matches) {
